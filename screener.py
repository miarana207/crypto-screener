...
def screen_spot(assets: List[Dict[str, Any]], quote_assets: Optional[set[str]] = None, warnings: Optional[List[str]] = None) -> Tuple[List[Dict[str, Any]], CriterionAudit]:
    ...
    # L'Order Book Depth est calculé après les données 24h et avant le prix minimum,
    # conformément à l'ordre de cascade demandé.
    if ORDER_BOOK_DEPTH_ENABLED:
        merge_spot_order_book_depth(assets, warnings)
        assets = apply_criterion(
            assets,
            audit,
            "7. Épaisseur carnet d'ordres minimum",
            lambda x: x.get("depthTotalQuote", 0.0) >= MIN_ORDER_BOOK_DEPTH_QUOTE,
        )
    else:
        # Le critère reste visible dans l'audit lorsque désactivé, sans filtrer.
        audit.add("7. Épaisseur carnet d'ordres minimum", len(assets), len(assets))

    assets = apply_criterion(assets, audit, "8. Prix minimum", lambda x: x["lastPrice"] >= MIN_PRICE)
    ...
    assets = apply_criterion(assets, audit, "9. Volume quote 24h minimum", lambda x: x["quoteVolume"] >= MIN_24H_QUOTE_VOLUME)
    assets = apply_criterion(assets, audit, "10. Nombre de trades minimum", lambda x: x["trades"] >= MIN_24H_TRADES)
    assets = apply_criterion(assets, audit, "11. Variation 24h minimum", lambda x: x["priceChangePercent"] >= MIN_24H_CHANGE_PERCENT)
    assets = apply_criterion(assets, audit, "12. Variation 24h maximum", lambda x: x["priceChangePercent"] <= MAX_24H_CHANGE_PERCENT)
    assets = apply_criterion(assets, audit, "13. Spread maximum", lambda x: x["spreadPercent"] is not None and x["spreadPercent"] <= MAX_SPREAD_PERCENT)
    return assets, audit
