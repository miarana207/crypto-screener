...
def run_technical_analysis(assets: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    untouched = [a for a in assets if a["market"] not in technical_markets]
...
def audit_to_html(audit: CriterionAudit) -> str:
    ...
    return "<table border='1' cellpadding='5' cellspacing='0'><tr><th>Critère</th><th>Avant</th><th>Retenus</th><th>Rejetés</th><th>Rétention</th><th>Rejet</th></tr>" + "".join(rows) + "</table>"
...
def build_report(market_results: Dict[str, List[Dict[str, Any]]], market_audits: Dict[str, CriterionAudit], errors: List[str], warnings: List[str]) -> str:
    now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    total = sum(len(v) for v in market_results.values())
    parts = ["<html><body>", "<h1>Binance Multi-Market Screener</h1>", f"<p><b>Date :</b> {now}</p>", f"<p><b>Total survivants :</b> {total:,}</p>", "<hr>"]
    ...
    parts.append(f"<h2>Paramètres principaux</h2><ul><li>Spot quotes : {', '.join(sorted(QUOTE_ASSETS))}</li><li>Spot volume minimum : {MIN_24H_QUOTE_VOLUME:,.0f}</li><li>Futures volume minimum : {MIN_FUTURES_QUOTE_VOLUME:,.0f}</li><li>Technical timeframe : {TECHNICAL_INTERVAL}</li><li>Technical max assets : {TECHNICAL_MAX_ASSETS}</li></ul>")
    if warnings:
        parts.append("<h2>Avertissements</h2><ul>" + "".join(f"<li>{w}</li>" for w in warnings) + "</ul>")
    if errors:
        parts.append("<h2>Erreurs</h2><ul>" + "".join(f"<li>{e}</li>" for e in errors) + "</ul>")
...
def main() -> int:
    ...
    if ENABLE_USDM:
        ...
        try:
            ...
            errors.append(f"USDⓈ-M FUTURES: {exc}")
    ...
    if ENABLE_COINM:
        ...
        try:
            ...
            errors.append(f"COIN-M FUTURES: {exc}")
    ...
    technical_results, technical_errors = run_technical_analysis(all_assets)
    ...
    print(f"Technical assets analyzed: {sum(len(v) for v in market_results.values()):,}")

    html = build_report(market_results, market_audits, errors, warnings)
