...
# Volatilité Spot : ATR(14) sur bougies 1h, appliquée après le spread.
VOLATILITY_ENABLED = os.getenv("VOLATILITY_ENABLED", "true").lower() == "true"
VOLATILITY_INTERVAL = os.getenv("VOLATILITY_INTERVAL", "1h")
VOLATILITY_ATR_PERIOD = int(os.getenv("VOLATILITY_ATR_PERIOD", "14"))
MIN_VOLATILITY_PERCENT = float(os.getenv("MIN_VOLATILITY_PERCENT", "0.75"))
_MAX_VOLATILITY_RAW = os.getenv("MAX_VOLATILITY_PERCENT", "").strip()
MAX_VOLATILITY_PERCENT = float(_MAX_VOLATILITY_RAW) if _MAX_VOLATILITY_RAW else None
...
def merge_spot_volatility(assets: List[Dict[str, Any]], warnings: Optional[List[str]] = None) -> None:
    """Calcule ATR% pour les survivants du spread avant le filtre de volatilité."""
    if not VOLATILITY_ENABLED or not assets:
        return

    workers = max(1, min(TECHNICAL_WORKERS, len(assets)))
    failures = 0

    def fetch(asset: Dict[str, Any]) -> Tuple[str, Optional[float]]:
        klines = spot_klines(asset["symbol"], VOLATILITY_INTERVAL, TECHNICAL_KLINES_LIMIT)
        if len(klines) < max(60, VOLATILITY_ATR_PERIOD + 1):
        ...
        atr_value = atr(highs, lows, closes, VOLATILITY_ATR_PERIOD)
        if atr_value is None or current <= 0:
            return asset["symbol"], None
        return asset["symbol"], atr_value / current * 100.0
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_map):
            asset = future_map[future]
            try:
                _, atr_percent = future.result()
                asset["volatilityAtrPercent"] = atr_percent
                asset["volatilityInterval"] = VOLATILITY_INTERVAL
                asset["volatilityAtrPeriod"] = VOLATILITY_ATR_PERIOD
            except Exception as exc:
                failures += 1
                asset["volatilityAtrPercent"] = None
                asset["volatilityError"] = str(exc)

    if failures and warnings is not None:
        warnings.append(f"Volatilité ATR Spot : {failures} échecs de récupération sur {len(assets)} paires.")


def build_volatility_diagnostics(assets: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Construit le diagnostic ATR% avant application du seuil minimum."""
    values = [
        as_float(asset.get("volatilityAtrPercent"), -1.0)
        for asset in assets
        if asset.get("volatilityAtrPercent") is not None
    ]
    ...
    return {
        ...
        "missing_count": len(assets) - len(values),
        "buckets": {
            "lt_0_25": sum(1 for value in values if value < 0.25),
            "0_25_0_50": sum(1 for value in values if 0.25 <= value < 0.50),
            "0_50_0_75": sum(1 for value in values if 0.50 <= value < 0.75),
            "0_75_1_00": sum(1 for value in values if 0.75 <= value < 1.00),
            "1_00_1_50": sum(1 for value in values if 1.00 <= value < 1.50),
            "1_50_2_00": sum(1 for value in values if 1.50 <= value <= 2.00),
            "gt_2_00": sum(1 for value in values if value > 2.00),
        },
...
def screen_spot(assets: List[Dict[str, Any]], quote_assets: Optional[set[str]] = None, warnings: Optional[List[str]] = None) -> Tuple[List[Dict[str, Any]], CriterionAudit]:
    ...
    # 10. Volatilité ATR(14) 1h minimum.
    # Le diagnostic est construit sur tous les survivants du spread AVANT le filtre.
    if VOLATILITY_ENABLED:
        merge_spot_volatility(assets, warnings)
        audit.diagnostics["volatility"] = build_volatility_diagnostics(assets)
        assets = apply_criterion(
            assets,
            audit,
            "10. Volatilité ATR(14) 1h minimum",
            lambda x: (
                x.get("volatilityAtrPercent") is not None
                and x["volatilityAtrPercent"] >= MIN_VOLATILITY_PERCENT
                and (MAX_VOLATILITY_PERCENT is None or x["volatilityAtrPercent"] <= MAX_VOLATILITY_PERCENT)
            ),
        )
    else:
        audit.diagnostics["volatility"] = build_volatility_diagnostics(assets)
        audit.add("10. Volatilité ATR(14) 1h minimum", len(assets), len(assets))
...
# ----------------------------- TECHNICAL -----------------------------
# Cache des klines Spot pendant le run. Le filtre de volatilité et l'analyse
# technique utilisent les mêmes données 1h et évitent ainsi un second appel API.
...
def audit_to_html(audit: CriterionAudit) -> str:
    ...
    volatility = audit.diagnostics.get("volatility")
    if volatility is not None:
        buckets = volatility.get("buckets", {})
        html += "<h3>Diagnostic volatilité 1h</h3>"
        html += f"<p>Univers : <b>{volatility.get('universe_count', 0):,}</b></p>"
        if volatility.get("missing_count", 0):
            html += f"<p>ATR% non disponible : <b>{volatility.get('missing_count', 0):,}</b></p>"
        html += "<table border='1' cellpadding='5' cellspacing='0'><tr><th>Plage ATR%</th><th>Actifs</th></tr>"
        html += f"<tr><td>ATR% &lt; 0,25</td><td>{buckets.get('lt_0_25', 0):,}</td></tr>"
        html += f"<tr><td>ATR% 0,25–0,50</td><td>{buckets.get('0_25_0_50', 0):,}</td></tr>"
        html += f"<tr><td>ATR% 0,50–0,75</td><td>{buckets.get('0_50_0_75', 0):,}</td></tr>"
        html += f"<tr><td>ATR% 0,75–1,00</td><td>{buckets.get('0_75_1_00', 0):,}</td></tr>"
        html += f"<tr><td>ATR% 1,00–1,50</td><td>{buckets.get('1_00_1_50', 0):,}</td></tr>"
        html += f"<tr><td>ATR% 1,50–2,00</td><td>{buckets.get('1_50_2_00', 0):,}</td></tr>"
        html += f"<tr><td>ATR% &gt; 2,00</td><td>{buckets.get('gt_2_00', 0):,}</td></tr>"
        html += "</table>"
        html += "<table border='1' cellpadding='5' cellspacing='0'><tr><th>Statistique</th><th>Valeur</th></tr>"
        for label, key in (("ATR% minimum", "minimum"), ("ATR% médian", "median"), ("ATR% moyen", "mean"), ("ATR% P25", "p25"), ("ATR% P75", "p75"), ("ATR% maximum", "maximum")):
            value = volatility.get(key)
            formatted = f"{value:.4f}%" if isinstance(value, (int, float)) else "-"
...
def market_summary_html(market: str, assets: List[Dict[str, Any]], audit: Optional[CriterionAudit]) -> str:
    ...
    if assets:
        html.append(f"<table border='1' cellpadding='5' cellspacing='0'><tr><th>Symbol</th><th>Volume</th><th>Depth ±{ORDER_BOOK_DEPTH_PCT:.2f}%</th><th>Spread</th><th>24h</th><th>Funding</th><th>Score technique</th><th>État</th></tr>")
        ...
        for asset in sorted_assets[:EMAIL_TOP_RESULTS]:
            ...
            html.append(f"<tr><td>{asset.get('symbol', '-')}</td><td>{format_number(asset.get('quoteVolume', 0))}</td><td>{format_number(asset.get('depthTotalQuote', 0), 0)}</td><td>{format_number(asset.get('spreadPercent', 0), 4)}%</td><td>{format_number(asset.get('priceChangePercent', 0))}%</td><td>{funding}</td><td>{format_number(asset.get('technicalScore', '-'))}</td><td>{asset.get('technicalState', '-')}</td></tr>")
        html.append("</table>")
    return "".join(html)
...
def build_report(market_results: Dict[str, List[Dict[str, Any]]], market_audits: Dict[str, CriterionAudit], errors: List[str], warnings: List[str], unavailable_markets: List[str], technical_analysis_count: int) -> str:
    ...
    parts.append(
        ...
        f"<li>Volatilité Spot : {'activée' if VOLATILITY_ENABLED else 'désactivée'} — ATR({VOLATILITY_ATR_PERIOD}) {VOLATILITY_INTERVAL} ≥ {MIN_VOLATILITY_PERCENT:.2f}%"
        f"{' — maximum désactivé' if MAX_VOLATILITY_PERCENT is None else f' — maximum {MAX_VOLATILITY_PERCENT:.2f}%'}"
        f"</li>"
