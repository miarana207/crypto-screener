...
CHOP_RANGE_THRESHOLD = float(os.getenv("CHOP_RANGE_THRESHOLD", "61.8"))
CHOP_TREND_THRESHOLD = float(os.getenv("CHOP_TREND_THRESHOLD", "38.2"))
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_map):
            ...
            try:
                ...
                asset.update(chop=None, adx=None, plusDI=None, minusDI=None, regimeAtrPercent=None, marketRegime="NON DISPONIBLE")
...
def classify_market_regime(chop_value: Optional[float], adx_value: Optional[float], atr_percent: Optional[float]) -> str:
    ...
    # Les zones extrêmes de CHOP et d'ADX doivent converger pour déclarer
    # proprement RANGE ou TENDANCE. Les configurations contradictoires restent
    # volontairement en TRANSITION plutôt que d'être forcées dans une catégorie.
    if chop_value >= CHOP_RANGE_THRESHOLD and adx_value < ADX_TREND_THRESHOLD:
        base = "RANGE"
    elif chop_value <= CHOP_TREND_THRESHOLD and adx_value >= ADX_TREND_THRESHOLD:
        base = "TENDANCE FORTE" if adx_value >= ADX_STRONG_THRESHOLD else "TENDANCE"
    else:
        base = "TRANSITION"
...
def analyze_klines(klines: List[List[Any]]) -> Dict[str, Any]:
    ...
    if ENABLE_ATR and atr_value is not None and current > 0:
        ...
        if atr_percent >= 2.0:
            ...
            score += 3
    return {"technicalScore": round(score, 2), "technicalState": classify_technical_state(score), "emaFast": ema_fast, "emaSlow": ema_slow, "rsi": rsi_value, "macd": macd_value, "macdSignal": macd_signal, "macdHistogram": macd_hist, "atr": atr_value, "volumeRatio": volume_ratio, "chop": chop_value, "adx": adx_value, "plusDI": plus_di, "minusDI": minus_di, "regimeAtrPercent": regime_atr_percent, "marketRegime": market_regime}
        ...
        if asset["market"] == "MARGIN":
            ...
            if source:
                for key in (
                    ...
                    "chop", "adx", "plusDI", "minusDI", "regimeAtrPercent", "marketRegime"
                ):
                    if key in source:
...
def build_report(market_results: Dict[str, List[Dict[str, Any]]], market_audits: Dict[str, CriterionAudit], errors: List[str], warnings: List[str], unavailable_markets: List[str], technical_analysis_count: int) -> str:
    ...
    parts.append(
        ...
        f"<li>Seuils d'interprétation : CHOP range ≥ {CHOP_RANGE_THRESHOLD:.1f}, CHOP tendance ≤ {CHOP_TREND_THRESHOLD:.1f}, ADX tendance ≥ {ADX_TREND_THRESHOLD:.1f}, ADX forte ≥ {ADX_STRONG_THRESHOLD:.1f}; intensité ATR : calme &lt; {ATR_LOW_PERCENT:.2f}%, active &lt; {ATR_HIGH_PERCENT:.2f}%, volatile &lt; {ATR_EXTREME_PERCENT:.2f}%, très volatile au-dessus</li>"
