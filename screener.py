...
def analyze_klines(klines: List[List[Any]]) -> Dict[str, Any]:
    ...
    market_direction = classify_market_direction(plus_di, minus_di, adx_value) if REGIME_ENABLED else "NON DISPONIBLE"
        ...
        if asset["market"] == "MARGIN":
            ...
            if source:
                for key in (
                    ...
                    "chop", "adx", "plusDI", "minusDI", "regimeAtrPercent", "marketNature", "marketStructure", "marketDirection", "marketRegime"
                ):
                    if key in source:
...
def market_summary_html(market: str, assets: List[Dict[str, Any]], audit: Optional[CriterionAudit]) -> str:
    ...
    if assets:
        html.append(f"<table border='1' cellpadding='5' cellspacing='0'><tr><th>Symbol</th><th>Volume</th><th>Depth ±{ORDER_BOOK_DEPTH_PCT:.2f}%</th><th>Spread</th><th>24h</th><th>CHOP</th><th>ADX</th><th>ATR%</th><th>Régime</th><th>Direction</th><th>Funding</th><th>Score technique</th><th>État</th></tr>")
        ...
        for asset in sorted_assets[:EMAIL_TOP_RESULTS]:
            ...
            html.append(f"<tr><td>{asset.get('symbol', '-')}</td><td>{format_number(asset.get('quoteVolume', 0))}</td><td>{format_number(asset.get('depthTotalQuote', 0), 0)}</td><td>{format_number(asset.get('spreadPercent', 0), 4)}%</td><td>{format_number(asset.get('priceChangePercent', 0))}%</td><td>{format_number(asset.get('chop', '-'), 2)}</td><td>{format_number(asset.get('adx', '-'), 2)}</td><td>{format_number(asset.get('regimeAtrPercent', '-'), 3)}%</td><td>{asset.get('marketRegime', '-')}</td><td>{asset.get('marketDirection', '-')}</td><td>{funding}</td><td>{format_number(asset.get('technicalScore', '-'))}</td><td>{asset.get('technicalState', '-')}</td></tr>")
        html.append("</table>")
    return "".join(html)
