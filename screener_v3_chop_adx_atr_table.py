...
def screen_spot(assets: List[Dict[str, Any]], quote_assets: Optional[set[str]] = None, warnings: Optional[List[str]] = None) -> Tuple[List[Dict[str, Any]], CriterionAudit]:
    ...
    # 10. Ancienneté minimale : au moins MIN_HISTORY_DAYS jours de bougies 1h disponibles.
    # Ce filtre est placé après le spread et avant le calcul du régime.
    required_history_bars = max(1, int(math.ceil(MIN_HISTORY_DAYS * 24))) if REGIME_INTERVAL == "1h" else max(1, int(math.ceil(MIN_HISTORY_DAYS * 24)))

    def has_minimum_history(asset: Dict[str, Any]) -> bool:
        try:
            klines = spot_klines(asset["symbol"], REGIME_INTERVAL, required_history_bars)
            return len(klines) >= required_history_bars
        except Exception as exc:
            asset["historyError"] = str(exc)
            return False

    assets = apply_criterion(assets, audit, "10. Ancienneté minimale", has_minimum_history)
...
def asset_category(asset: Dict[str, Any]) -> str:
    ...
    2) liste explicite BINANCE_TOKENIZED_SPOT_BASES ;
    3) SPOT / CRYPTO pour les autres actifs Spot.

    Une liste explicite est acceptable ici car elle ne repose pas sur une règle
    implicite telle que ``baseAsset.endswith("B")``.
    """
...
def audit_to_html(audit: CriterionAudit) -> str:
    ...
    if regime is not None:
        ...
        family_counts = regime.get("regime_counts", {})
