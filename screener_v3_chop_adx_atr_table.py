...
TOKENIZED_SPOT_BASES = {
    x.strip().upper()
    for x in os.getenv("BINANCE_TOKENIZED_SPOT_BASES", "").split(",")
    if x.strip()
}
...
def asset_category(asset: Dict[str, Any]) -> str:
    """Classe le Spot sans confondre suffixe de ticker et tokenisation.

    Aucune règle du type ``baseAsset.endswith("B")`` n'est utilisée : elle
    classerait à tort des actifs comme BNB. La catégorie TOKENIZED / SPECIAL
    n'est produite que sur indication explicite.
    """
    if asset.get("market") != "SPOT":
        return asset.get("market", "AUTRE")

    explicit_flags = (
        asset.get("isTokenized"),
        asset.get("tokenized"),
        asset.get("isTokenizedAsset"),
    )
    if any(value is True for value in explicit_flags):
        return "TOKENIZED / SPECIAL"

    base = str(asset.get("baseAsset", "")).upper()
    if base in TOKENIZED_SPOT_BASES:
        return "TOKENIZED / SPECIAL"

    return "SPOT / NON CLASSÉE"
...
def spot_chop_adx_atr_table_html(assets: List[Dict[str, Any]]) -> str:
    ...
    # On regroupe les actifs par lecture finale + intensité ATR + direction.
    groups = {}
    for asset in assets:
        ...
        key = (family, intensity, direction)
        g = groups.setdefault(key, {"count": 0, "symbols": [], "chop": [], "adx": [], "atr": []})
        g["count"] += 1
        symbol = asset.get("symbol")
        if symbol and len(g["symbols"]) < 6:
            g["symbols"].append(symbol)
        for dst_key, src_key in (("chop", "chop"), ("adx", "adx"), ("atr", "regimeAtrPercent")):
            value = as_float(asset.get(src_key))
            if value is not None:
                g[dst_key].append(value)

    family_order = {
        "TENDANCE FORTE": 0,
        "TENDANCE": 1,
        "TRANSITION DIRECTIONNELLE": 2,
        "TRANSITION NEUTRE": 3,
        "CONFLIT": 4,
        "RANGE": 5,
        "NON DISPONIBLE": 6,
    ...
    ordered = sorted(
        ...
        key=lambda item: (
            ...
            intensity_order.get(item[0][1], 99),
    ...
    # La ligne CHOP/ADX est reconstruite depuis la classification finale afin que
    # le tableau soit directement cohérent avec les règles du screener.
    for (family, intensity, direction), g in ordered:
        chop_values = g["chop"]
        adx_values = g["adx"]
        atr_values = g["atr"]
        if family in ("TENDANCE FORTE", "TENDANCE", "TRANSITION — TENDANCE NAISSANTE"):
            chop_label = "≤ 40"
        elif family in ("TRANSITION DIRECTIONNELLE", "TRANSITION NEUTRE"):
            chop_label = "40–55"
        elif family == "RANGE" or family == "CONFLIT":
            chop_label = "≥ 55" if family == "RANGE" else "≥ 55"
        else:
            chop_label = "—"

        if family == "TENDANCE FORTE":
            adx_label = "≥ 40"
        elif family == "TENDANCE":
            adx_label = "25–40"
        elif family == "TRANSITION — TENDANCE NAISSANTE":
            adx_label = "< 25"
        elif family == "TRANSITION DIRECTIONNELLE":
            adx_label = "≥ 25"
        elif family == "TRANSITION NEUTRE":
            adx_label = "< 25"
        elif family == "RANGE":
            adx_label = "< 25"
        elif family == "CONFLIT":
        ...
        html.append(
            "<tr>"
            f"<td>{html_escape(chop_label)}</td>"
            f"<td>{html_escape(adx_label)}</td>"
            f"<td>{html_escape(intensity)}</td>"
            f"<td><b>{html_escape(family)}</b></td>"
            f"<td>{html_escape(direction)}</td>"
            f"<td style='text-align:right'><b>{g['count']}</b></td>"
            f"<td>{format_number(mean(chop_values), 2)}</td>"
            f"<td>{format_number(mean(adx_values), 2)}</td>"
            f"<td>{format_number(mean(atr_values), 3)}%</td>"
...
def spot_base_asset_summary_html(assets: List[Dict[str, Any]]) -> str:
    ...
    for asset in assets:
        ...
        if not base:
            continue
        g = groups.setdefault(base, {"pairs": 0, "volume": 0.0, "directions": {}, "families": {}, "symbols": []})
        g["pairs"] += 1
    ...
    html = [
        ...
        "<tr><th>Base</th><th>Paires</th><th>Volume cumulé</th><th>Direction dominante</th><th>Lecture dominante</th><th>Paires représentatives</th></tr>",
    ]
    for base, g in ordered[:limit]:
        dominant_direction = max(g["directions"].items(), key=lambda x: x[1])[0] if g["directions"] else "NEUTRE"
        dominant_family = max(g["families"].items(), key=lambda x: x[1])[0] if g["families"] else "NON DISPONIBLE"
        html.append(
            "<tr>"
            f"<td><b>{html_escape(base)}</b></td><td>{g['pairs']}</td>"
...
def build_report(market_results: Dict[str, List[Dict[str, Any]]], market_audits: Dict[str, CriterionAudit], errors: List[str], warnings: List[str], unavailable_markets: List[str]) -> str:
    ...
    if summary:
        ...
        parts.append("<p><i>Classification Spot : aucune paire n'est déclarée TOKENIZED sur la seule base de son ticker. Sans métadonnée explicite ou liste déclarée, elle reste <b>SPOT / NON CLASSÉE</b>.</i></p>")
        parts.append(
